...
import TransportShip from './units/TransportShip'
            ...
            if (unit.selected) {
                for (const i in client.globalUnits) {
                    ...
                    if (longClick(p.RIGHT) && otherUnit.sprite.position.dist(mousePos) < 80) {
                        ...
                        if(otherUnit.type === "transport ship" && otherUnit.faction === unit.faction && unit.type === "infantry") {
                            unit.embarking = true
                        }
                        else{
