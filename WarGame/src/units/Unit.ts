...
class Unit {
        ...
        if (this.sprite.collisions.length !== 0) {
            for (const i in this.sprite.collisions) {
                ...
                if (colliding instanceof Unit) {
                    ...
                    if (this.sprite.position.dist(colliding.sprite.position) < 8 && this.faction === colliding.faction && !this.sprite.position.equals(colliding.sprite.position) && !this.combining) {
                        if (this.type === colliding.type) {
                            this.combine(colliding)
                        }
                        else if (colliding.is<TransportShip>("transport ship") && this.embarking) {
                            colliding.embark(this)
                        }
                    }
                }
    ...
    updateLabels() {
        ...
        p.fill(255, 255, 255)
        p.text(this.faction, pos.x, pos.y - 12.5)
        p.textSize(8)
